...
# - 반대로 가장 상단에 정의되어 프로그램 종료 전까지 유지 되는 변수를 전역 변수
# - 같은 이름의 지역변수, 전역변수 존재하는 경우 가까운(지역 변수)가 우선순위가 더 높음 """
...
# - 인자(parameter) : 함수 입력 값
# - 함수 입력값의 개수가 함수를 호출할 때마다 매번 다른 경우
# -> 가변 길이 인자를 사용해야함!!!
...
def test2(**kwargs):
    for key, value int kwargs.items():
        print(key,value)
test(a=1,b=2,c=3)